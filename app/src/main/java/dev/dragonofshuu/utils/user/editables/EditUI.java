...
public class EditUI {
    ...
    public EditUI(ArrayList<EditableItem> items) {
        this.items = items;
    }
    ...
    private ExitType mainLoop(Scanner scan, String question, EditableItem baseClass, Player player) {
        while (true) {
            ...
            /* Resets the chosen item */
            returnItem = null;

            /* Put favorites at the top */
            items = distinctFavorites(items);

            /* Allocates the corrent amount of space for the User choices */
            String[] choosable;
            ...
            /* Gets all the items together as strings */
            for (int i = 0; i < items.size(); i++) {
                if (items.get(i) == null) continue;
                choosable[i] = (items.get(i).isFavorite() ? "★ " : "") + items.get(i).toString();
            }

            /* Add a Create New... if the object is a complete Object Addition */
            if (baseClass.isCompleteObjectAddition()) {
                choosable[choosable.length-2] = "Create new...";
            }
            /* Add the ability to exit the UI */
            choosable[choosable.length-1] = "Exit";

            /* Ask the user what item to target */
    ...
    private ResolveType favorite(EditableItem eItem) {
        eItem.toggleFavorite();
        return ResolveType.CONTINUE;
    }
    ...
    public ArrayList<EditableItem> distinctFavorites(ArrayList<EditableItem> items) {
        ArrayList<EditableItem> returnable = new ArrayList<EditableItem>();

        for (EditableItem item : items) {
            if (item.isFavorite()) returnable.add(0, item);
            else returnable.add(item);
        }
